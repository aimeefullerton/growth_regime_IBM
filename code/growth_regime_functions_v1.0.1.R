...
#         coldwater fishes.
#
# Last functional updates 13 June 2019; editorial edits 15 October 2020
#*************************************************************************************************
...
distance2base4fish <- function(fishID = 1, ssn = ssn1) {
  return(fish$length2segBase[fish$pid == fishID] - ssn@data[fish$seg[fish$pid == fishID], length.field]
         + distance2base4seg(fish$seg[fish$pid == fishID]))
}
...
# Generate initial stream temperature landscape pattern
#   Used when no data are available (i.e., from GIS or models)
fncInitializeWT <- function(tr = tr730[1, "WT"] - 3, ssn = ssn1, plotit = "file"){
  ...
  # using exponential relationship to keep headwaters cool and mainstems warm
  z <- log(tmpvar[,so.field] + (1 - tmpvar[,"ratio"]))
  v <- fncRescale(z ^ 2, c(0, 1))

  tmpvar[,wt.field] <- (runif(nrow(tmpvar), 15, 20) * v) + 8  #runif adds some stochasticity 
  ...
  #first, subtract min. next, multiply temp on day from climate scenario by scaled WT. third, add min back in.
  #gives values ranging across the network from min to tr instead of from zero to tr
  tmpvar[,wt.field] <- (tr - min(tmpvar[,wt.field])) * tmpvar$scaledWT + min(tmpvar[,wt.field])
  ...
  if(plotit != "none"){
    ...
    plot(ssn, "WT", breaktype = "user", brks = seq(5, 27, length.out = 11), lwdLineCol = "addfunccol", lwdLineEx = 8, 
         lineCol = "darkgray", xlab = "X", ylab = "Y", color.palette = cb)
    if(plotit == "file") {dev.off()}
  }
...
#   Used when predictions are based on fncInitializeWT(); not used with empirical or modeled data
fncUpdateWaterTemps <- function(ts, WTu, tr = tr730, ssn = ssn1, type = "Obs", diel = T, plotit = "file"){
  #WTu is a matrix of spatially distributed water temperatures from ssn
  #ts is the timestep; tr is the thermal regime scenario at the warmest location
  
  WTcc.jd <- tr[,"WT"][tr[,"JD"] == ts]
  ind <- which(tr[,"JD"] == ts)
  ifelse(ts - floor(ts) < 0.2, night <- TRUE, night <- FALSE)
  daily.chg <- ifelse(night == T, (WTcc.jd - tr[ind - 1, "WT"]) / 2, (WTcc.jd - tr[ind - 2, "WT"]) / 2)
  ...
  if(plotit!= "none"){
    ...
    plot(ssn, "WT", breaktype = "user", brks = c(0, seq(5, 29, length.out = 9), 34), lwdLineCol = "addfunccol", lwdLineEx = 8,
         lineCol = "darkgray", xlab = "X", ylab = "Y", color.palette = cb)
    if(plotit == "file"){dev.off()}
  }
...
# Calculate the density of species1, species2, and total fish in a segment that a species1 fish inhabits
fncFishDensity <- function(fish1.seg = fish1[,"seg"], fish2.seg = fish2[,"seg"], corr.factor = corr.factor,
  ...
  # multiply by 1000 because length is in km, width is in m, area is in m^2
  ...
  density <- matrix(c(density.species1, density.species2, density.total), nrow = num.fish1, ncol = 3)
  density<- density * corr.factor 
  #applying the correction factor treats each individual like a "superindividual" to reduce the resources required
...
# Estimate the functionally useable width of each segment
...
fncMoveOneStep <- function(ssn, plotit = "none") {
  ...
  ifelse(length(grep("network-",netnm)) > 0, locs <- ddply(fish, .(pid), function(x) data.frame(getXY(x$seg, x$ratio))), 
         locs <- ddply(fish, .(pid), function(x) data.frame(getXY.arc(x$seg, x$ratio))))
  fish$xloc <- locs$xloc
  fish$yloc <- locs$yloc
...
# Constants, hard-coded for O. mykiss - using RATION instead of p-values
